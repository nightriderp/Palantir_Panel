...
| `BACKUP_CHECKSUM_MISMATCH` | 422 | Gespeicherte SHA-256 des Backups weicht beim Restore/Download vom real gelesenen Archiv ab – beschädigt oder verändert (§14) |
...
| `AGENT_FILE_EXISTS` | 409 | `FILE_UPLOAD` trifft auf einen belegten Zielpfad ohne `overwrite` (Datei-Manager, §5.3) |
...
**Prüfsummen-Verifikation bei Restore/Download (Ergänzung aus N4, Fundpunkt 99):** Die beim Anlegen über das fertige Archiv gebildete `checksumSha256` (`CreateBackupCommandResult`) wird bei `RESTORE_BACKUP` und `DOWNLOAD_BACKUP` gegen das real gelesene Archiv geprüft; eine Abweichung meldet `BACKUP_CHECKSUM_MISMATCH` (§5.1, 422). Die Prüfung sitzt jeweils dort, wo das Archiv gelesen wird. **Restore:** im Agent (A3) – nur er hält die Bytes; das Backend reicht die erwartete Prüfsumme als `expectedChecksum` in der Nutzlast mit, der Agent verifiziert **vor** dem Anhalten des Containers und dem Leeren des Datenordners, sodass ein beschädigtes Archiv keinen laufenden Server stört und keinen halben Stand hinterlässt. **Download:** im Backend (B5) – das Archiv fließt beim blockweisen Zusammensetzen ohnehin vollständig durch das Backend; die Prüfsumme läuft strömend mit und wird vor dem letzten Block verglichen, sodass ein beschädigtes Archiv nie als vollständige, gültige Datei ausgeliefert wird (der Stream bricht ab, die zugesagte `content-length` wird nicht erreicht). Beide Wege puffern das Archiv nicht als Ganzes im Speicher.
...
**`FILE_DELETE` und `FILE_UPLOAD` (Ergänzung aus WELLE 0, Arbeitspaket P2):** Der Datei-Manager (Lastenheft §3.3) konnte bisher auflisten, lesen und schreiben (`FILE_LIST`/`FILE_READ`/`FILE_WRITE`), aber weder löschen noch hochladen. Beide Befehle vervollständigen das Wire-Format je Nutzdaten und Ergebnis (`FileDeleteCommandPayload`/`FileUploadCommandPayload`, Ergebnis jeweils `null`), damit P2 Backend und Agent dagegen bauen kann. `FILE_DELETE` ist idempotent (fehlender Pfad ist kein Fehler) und löscht ein nicht-leeres Verzeichnis nur mit `recursive: true`. `FILE_UPLOAD` legt eine hochgeladene – auch binäre, auch neue – Datei ab (Inhalt Base64-kodiert wie bei `FILE_WRITE`), prüft die Größe gegen `MAX_UPLOAD_SIZE_BYTES` (§12.1, `AGENT_FILE_TOO_LARGE`) und lehnt einen belegten Zielpfad ohne `overwrite` mit `AGENT_FILE_EXISTS` ab. **Seit P2 werden beide ausgeführt** (`ContainerRuntime.deleteFile`/`uploadFile`, Docker- und Fake-Variante) und stehen in `IMPLEMENTED_AGENT_COMMANDS`. Beide Pfade laufen wie die übrigen Dateibefehle durch `resolveWithinRoot()` gegen das Datenvolume des Containers.

**Grenze von `FILE_DELETE`:** Die Engine-API bietet zum Lesen und Schreiben den Archiv-Endpunkt, aber kein Löschen – der Agent führt dafür `rm`/`rmdir` als Argumentliste ohne Shell im Container aus. Das setzt einen **laufenden** Container voraus; auf einem gestoppten Server scheitert der Befehl. Auflisten, Lesen, Schreiben und Hochladen sind davon nicht betroffen. Anschluss dazu: WORK_STATUS.md, Gefundener Punkt 105.
...
- **Ergänzungen aus B7 (Chat & Moderation, §15):** `message.sent`, `message.deleted`, `conversation.created` und `conversation.read`. Sie fließen ausschließlich über den Chat-Kanal des Browsers (`packages/contracts/src/chat.ts`) und halten eine offene Ansicht aktuell – wie die Live-Ereignisse der Server-Ansicht sind sie bewusst **kein** Anlass für eine `NotificationRule`. Anlass für eine Benachrichtigung bleibt allein `message.reported`. `conversation.read` meldet den serverseitigen Lesezustand (§15) und wird ausschließlich an die weiteren Verbindungen desselben Kontos zugestellt, damit dessen Ungelesen-Zähler geräteübergreifend konsistent bleibt.
...
- **Serverseitiger Lesezustand (`POST /api/chat/conversations/:conversationId/read`, Fundpunkt 95):** Der Lesestand wird je Teilnehmer serverseitig geführt (`conversation_reads`: Konversation, Konto, `lastReadAt`), damit der Ungelesen-Zähler geräteübergreifend gilt und nicht nur lokal in einer Sitzung. `ConversationDto` trägt dafür additiv `unreadCount` und `lastReadAt`. Ungelesen ist eine Nachricht, die nach `lastReadAt` entstanden ist, nicht vom Aufrufer selbst stammt und nicht gelöscht ist. Der Endpunkt setzt `lastReadAt` auf den Serverzeitpunkt, liefert die aktualisierte Konversation zurück und stellt `conversation.read` an die **weiteren** Verbindungen desselben Kontos zu (kein Polling). Er läuft über den Teilnehmerweg (keine Permission): Wer nicht teilnimmt, erhält `CONVERSATION_NOT_FOUND` und kann so den Lesestand fremder Konversationen weder setzen noch ableiten. Das Datenschutz-Prinzip aus `visibility.ts` bleibt unangetastet – der Lesezustand ist rein privat und für Moderatoren nirgends sichtbar.
- **DM-Empfängerverzeichnis (`GET /api/chat/recipients`, `DirectMessageRecipientDto`):** Um eine Direktnachricht zu beginnen, braucht `openDirectConversation` eine `recipientId`. Wer dafür in Frage kommt, ist bewusst **eng** gefasst und folgt demselben Datenschutz-Prinzip wie der Rest des Moduls: Das Verzeichnis liefert **nicht** alle Konten der Plattform, sondern nur Besitzer und Mitglieder der Server, auf die der Aufrufer selbst Zugriff hat (Besitzer oder Mitglied) – freigeschaltet, nicht gesperrt, ohne das eigene Konto. Der Teilnehmerkreis wird wie überall frisch gelesen; wer aus allen gemeinsamen Servern entfernt wurde, verschwindet aus dem Verzeichnis. Wer mit niemandem einen Server teilt, erhält eine leere Liste und kann keine DM „ins Blaue" beginnen. Die Regel steht als Prädikat und Mengenbildung an genau einer Stelle (`visibility.ts`: `isDirectRecipientAllowed`, `directRecipientCandidateIds`). Bewusst **kein** globaler `GET /users`-Endpunkt, der die Kontenliste quer offenlegen würde.
