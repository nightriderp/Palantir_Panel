...
import { AGENT_VERSION, versionMitStand } from './version.js';

/**
 * Der Agent meldet seine Version im `hello`-Frame; das Backend zieht sie zur
...
describe('AGENT_VERSION', () => {
  ...
  it('ist nicht der Ersatzwert und hat die Form einer Versionsnummer', () => {
...
/**
 * Der ausgerollte Stand an der Version (Fundpunkt 318).
 *
 * Die Paketversion steht seit jeher auf 0.6.0 und aendert sich mit keinem
 ...
 * mitgibt, soll die Version nicht verlaengern.
 */
describe('versionMitStand', () => {
  it('haengt den Commit als Baumetadaten an', () => {
    expect(versionMitStand('0.6.0', 'dce821c772360bbe0b9123632150684d47403b3e')).toBe(
      '0.6.0+dce821c77236',
    );
  });

  it('kuerzt auf zwoelf Stellen und nimmt auch schon gekuerzte Werte', () => {
    expect(versionMitStand('0.6.0', 'dce821c77236')).toBe('0.6.0+dce821c77236');
    expect(versionMitStand('0.6.0', 'dce821c')).toBe('0.6.0+dce821c');
  });

  it('laesst die Paketversion stehen, wenn nichts Brauchbares kommt', () => {
    for (const wert of [undefined, '', '   ', 'beliebiger text', 'zzzz', 'abc']) {
      expect(versionMitStand('0.6.0', wert)).toBe('0.6.0');
    }
  });
