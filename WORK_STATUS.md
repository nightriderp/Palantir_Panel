...
| 7   | Projekt / Datenbank      | lokale Entwicklungsumgebung         | PostgreSQL 18 läuft als Windows-Dienst auf Port 5432, aber Rolle und Datenbank `palantir` existieren noch nicht und `DATABASE_URL` enthält weiterhin `CHANGE_ME`. Muss der Betreiber anlegen (Superuser-Passwort erforderlich), bevor F0 die erste Migration ausführen kann.                                              | offen                 |
