...
| 7   | Projekt / Datenbank      | lokale Entwicklungsumgebung         | PostgreSQL 18 läuft als Windows-Dienst auf Port 5432, aber Rolle und Datenbank `palantir` existieren noch nicht und `DATABASE_URL` enthält weiterhin `CHANGE_ME`. Muss der Betreiber anlegen, bevor F0 die erste Migration ausführen kann – am einfachsten über `scripts/dev-db-setup.ps1` (fragt einmal nach dem postgres-Superuser-Passwort). Prod-Weg über den DB-Container steht in SETUP.md §2.1. | offen                 |
